...
(`POST /v1beta/interactions`) for Nano Banana 2 and requests `mime_type: "image/jpeg"` with
`delivery: "uri"` — the only combination the `ImageResponseFormat` contract accepts — so the runtime
performs a second outbound HTTPS request to a Google-hosted media origin
(`generativelanguage.googleapis.com`, `*.googleapis.com`, `*.googleusercontent.com`) to download
the generated bytes. Egress filtering must allow those hosts. The transient download URI is never
logged or persisted, and the API key is presented only to `generativelanguage.googleapis.com`.
`FIREMARK_GENERATION_TIMEOUT_SECONDS` bounds both requests, and
`FIREMARK_MAX_GENERATED_IMAGE_BYTES` bounds the downloaded image.
