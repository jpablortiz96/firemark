...
`GEMINI_API_KEY` is a Google AI Studio key. Image generation calls the Google Interactions API
(`POST /v1beta/interactions`) for Nano Banana 2 with `mime_type: "image/jpeg"` and reads the image
inline from the response. Exactly one outbound host is contacted —
`generativelanguage.googleapis.com` — so egress filtering needs no other allowance and the API key
never reaches another origin. `FIREMARK_GENERATION_TIMEOUT_SECONDS` sizes the read timeout for one
generation (connect, write and pool are capped at 30 s), and `FIREMARK_MAX_GENERATED_IMAGE_BYTES`
bounds both the decoded image and, indirectly, the streamed JSON body.

The downloaded JPEG is normalized to the sealed PNG carrier in-process through pinned Pillow. The
runtime image therefore includes `Pillow==11.3.0`; normalization performs no network access and
needs no additional environment variable.
